...
public:
		...
		int 		Match(int prediction, bool skip = false, bool color = true);
...
const	results&	history() const;
		...
		int			amount() const;
private:
		throw_ 		Throw(int prediction, bool color) const;
		throw_ 		ThrowSkip(int prediction, bool color) const;
		void 		PrintDice(std::vector<int> const &result) const;
		void 		PrintInfo(int prediction, int sum, bool color) const;
